...
import { fileToContent, isDataUrl, isVideoName } from "@/lib/device";
import { HOME, listDir } from "@/lib/fs";
import { useMoor } from "@/lib/store";

const DIR = `${HOME}/Videos`;
  ...
  const clips = useMemo(
    () =>
      (listDir(fs, DIR) ?? []).filter(
        (e) => e.node.kind === "file" && (isVideoName(e.name) || isDataUrl(e.node.content)),
      ),
    [fs],
  );
  const playing = clips.find((t) => t.name === current);

  return (
    <AppShell
      bar={
        <>
          <span className="flex-1 text-xs text-muted">{clips.length} clips</span>
    ...
    >
      ...
      {playing?.node.kind === "file" && isDataUrl(playing.node.content) ? (
        <video src={playing.node.content} controls autoPlay className="max-h-64 w-full bg-crust" />
      ) : null}
      <ul className="p-2">
        {clips.length === 0 ? (
          <li className="px-3 py-10 text-center text-sm text-muted">Import video from the phone to play it here.</li>
        ) : (
          clips.map((t) => (
            <li key={t.name}>
              <button
